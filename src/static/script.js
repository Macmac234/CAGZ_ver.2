    ...
    navLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            if (targetSection) {
                ...
                // Adjust scroll speed by adding a slight delay before the next scroll event
                // This is a workaround as scrollIntoView does not directly support speed control
                // For more precise control, a custom scrolling function would be needed.
                setTimeout(() => {}, 50); // Small delay to allow scroll to complete slightly faster
            }
        });
    });
...
async function registerPlayer() {
    ...
    if (!name || !phone || !password) {
        feedback.innerHTML = '<span style="color: #dc3545;">Please enter name, phone number, and password!</span>';
        return;
    }
    
    // Check if phone number already exists
    const existingPlayers = getPlayersFromStorage();
    const phoneExists = existingPlayers.some(player => player.phone === phone);
    
    if (phoneExists) {
        // Show warning and switch to continue mode
        document.getElementById('phone-warning').style.display = 'block';
        document.getElementById('existing-phone-display').textContent = phone;
    ...
    try {
        ...
        if (response.ok) {
            ...
            savePlayerToStorage({ name, password, phone });
...
function continueRegistration() {
    // Allow registration with existing phone number
    const nameInput = document.getElementById('player-name-input');
    const phoneInput = document.getElementById('player-phone-input');
    const passwordInput = document.getElementById('player-password-input');
    const feedback = document.getElementById('registration-feedback');
...
function findPlayersByPhone(phone) {
    const players = getPlayersFromStorage();
    return players.filter(player => player.phone === phone);
}

function updatePlayerPassword(playerName, phone, newPassword) {
    const players = getPlayersFromStorage();
    const playerIndex = players.findIndex(p => p.name === playerName && p.phone === phone);
    if (playerIndex !== -1) {
...
function loginPlayer() {
    ...
    if (player) {
        currentPlayer = { name: player.name, phone: player.phone };
...
function showForgotPasswordModal() {
    ...
    // Clear previous inputs
    document.getElementById('search-phone-input').value = '';
...
function searchByPhone() {
    ...
    if (!phone) {
        feedback.innerHTML = '<span style="color: #dc3545;">Please enter a phone number!</span>';
        return;
    }
    
    const players = findPlayersByPhone(phone);
    
    if (players.length === 0) {
        feedback.innerHTML = '<span style="color: #dc3545;">No players found with this phone number!</span>';
        return;
    }
    
    // Show search results modal
    showSearchResultsModal(phone, players);
}

// Search Results Modal Functions
function showSearchResultsModal(phone, players) {
    closeAllModals();
    const modal = document.getElementById('search-results-modal');
    modal.classList.add('active');
    document.body.style.overflow = 'hidden';
    
    document.getElementById('searched-phone-display').textContent = phone;
    
    const playersList = document.getElementById('player-names-list');
    playersList.innerHTML = '';
    
    players.forEach(player => {
        const playerButton = document.createElement('button');
        playerButton.textContent = player.name;
        playerButton.className = 'game-button';
        playerButton.style.cssText = 'display: block; margin: 0.5rem 0; width: 100%;';
        playerButton.onclick = () => selectPlayer(player);
...
function closeSearchResultsModal() {
    ...
    modal.classList.remove("active");
    document.body.style.overflow = "auto";
    document.getElementById("search-phone-input").value = ""; // Clear the search input
}
...
function sendOTP() {
    if (!selectedAction) {
        const feedback = document.getElementById('selection-feedback');
        feedback.innerHTML = '<span style="color: #dc3545;">Please choose an action first!</span>';
        return;
    }
    ...
    const feedback = document.getElementById('selection-feedback');
    feedback.innerHTML = `<span style="color: #28a745;">OTP sent to ${selectedPlayerData.phone}: ${generatedOTP}</span>`;
}
...
function verifyOTP() {
    ...
    if (enteredOTP !== generatedOTP) {
        const feedback = document.getElementById('selection-feedback');
        feedback.innerHTML = '<span style="color: #dc3545;">Invalid OTP! Plefunction selectPlayerAction(action) {
    selectedAction = action;
...
function updatePassword() {
    ...
    const success = updatePlayerPassword(selectedPlayerData.name, selectedPlayerData.phone, newPassword);
...
function showOtpVerificationModal() {
    ...
    document.getElementById("otp-countdown").style.display = "none";
    document.getElementById("send-otp-btn").style.display = "block";
...
function sendOTP() {
    if (!selectedPlayerData) {
        document.getElementById("otp-feedback").innerHTML = "<span style=\"color: #dc3545;\">No player selected.</span>";
        return;
    }

    generatedOTP = Math.floor(1000 + Math.random() * 9000).toString(); // Generate a 4-digit OTP
    console.log(`Mock OTP for ${selectedPlayerData.phone}: ${generatedOTP}`); // Log OTP for testing

    document.getElementById("otp-feedback").innerHTML = `<span style=\"color: var(--primary-color);\">OTP sent to ${selectedPlayerData.phone}. Mock OTP: ${generatedOTP}</span>`;
    document.getElementById("send-otp-btn").style.display = "none";
    startOTPCountdown();
}
...
function startOTPCountdown() {
    ...
    countdownElement.textContent = `Resend OTP in ${timeLeft}s`;
    ...
    otpCountdownInterval = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            ...
            otpResendAttempts++;
            if (otpResendAttempts >= 3) {
                document.getElementById("send-otp-btn").disabled = true;
                document.getElementById("otp-feedback").innerHTML = "<span style=\"color: #dc3545;\">Too many resend attempts. Please try again later.</span>";
            }
        }
    }, 1000);
}
...
function verifyOTP() {
    ...
    if (otpInput === generatedOTP) {
        feedback.innerHTML = "<span style=\"color: #28a745;\">OTP verified successfully!</span>";
        clearInterval(otpCountdownInterval);
        setTimeout(() => {
            if (selectedAction === 'view') {
                showPasswordShowModal(selectedPlayerData.password);
            } else if (selectedAction === 'change') {
                showChangePasswordModal();
            }
        }, 1000);
    } else {
        feedback.innerHTML = "<span style=\"color: #dc3545;\">Invalid OTP. Please try again.</span>";
    }
}
